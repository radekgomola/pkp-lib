...
<form class="pkp_form" id="identityForm" method="post" action="{url op="saveIdentity"}" enctype="multipart/form-data">
    ...
    {/fbvFormArea}
    <hr />
    {fbvFormArea id="userZlatyFond" }
            <h2>{translate key="registration.zlatyFond"}</h2>
            {fbvFormSection title="registration.zlatyFond.description" list=true}
                    
                    {if $zlatyFondPristup}
                        {assign var="checked" value=true}
                        {translate key="registration.zlatyFond.trvani" datum={$zlatyFondDate}}
                        {fbvElement type="checkbox" name="zlatyFond" id="zlatyFond" value=0 checked=$checked label=$registration.zlatyFond.checkbox translate=false}
                    {else}
