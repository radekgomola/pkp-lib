...
class SubmissionFileManager extends BaseSubmissionFileManager {
     ...
     * @return boolean
     */
    function downloadFile($fileId, $revision = null, $inline = false, $filename = null, $flipbook = false) {
        $returner = false;
        $submissionFile = $this->_getFile($fileId, $revision);
        if (isset($submissionFile)) {
            ...
            if ($flipbook && $filename) {
                ...
                $filePath = $submissionFile->getFlipbookFolderPath() . "/" . $filename;
                $mediaType = null;
            } else {
                // Send the file to the user.
    ...
    function recordView($submissionFile) {
        ...
        if (is_a($user, 'User')) {
            ...
            $viewsDao->recordView(
                    ASSOC_TYPE_SUBMISSION_FILE, $submissionFile->getFileIdAndRevision(), $user->getId()
            );
        }
    }
    ...
    function temporaryFileToSubmissionFile($temporaryFile, $fileStage, $uploaderUserId, $uploaderUserGroupId, $revisedFileId, $genreId, $assocType, $assocId, $flipbookChecker) {
        ...
        if (!$submissionFileDao->insertObject($submissionFile, $sourceFile))
            return false;
    ...
    function copyFileToFileStage($sourceFileId, $sourceRevision, $newFileStage, $destFileId = null, $viewable = false) {
        if (HookRegistry::call('SubmissionFileManager::copyFileToFileStage', array(&$sourceFileId, &$sourceRevision, &$newFileStage, &$destFileId, &$result)))
            return $result;
     ...
     */
    function _handleUpload($fileName, $fileStage, $uploaderUserId, $uploaderUserGroupId, $revisedFileId = null, $genreId = null, $assocType = null, $assocId = null, $flipbookChecker = null) {
    ...
    function _instantiateSubmissionFile($sourceFilePath, $fileStage, $revisedFileId = null, $genreId = null, $assocType = null, $assocId = null, $flipbookChecker = null) {
        ...
        if ($fileStage != SUBMISSION_FILE_REVIEW_ATTACHMENT) {
            ...
            if (!$genreId || $revisedFileId) {
                ...
                if (!is_a($revisedFile, 'SubmissionFile'))
                    return null;
            }
        }
        ...
        if ($revisedFileId) {
            ...
            if ($revisedFile->getSubmissionId() != $this->getSubmissionId())
                return null;
            ...
            if (!is_null($fileStage) && $fileStage !== $revisedFile->getFileStage()) {
                ...
                // Copy the assoc type.
                if (!is_null($assocType) && $assocType !== $revisedFile->getAssocType())
                    fatalError('Invalid submission file assoc type!');
                $assocType = (int) $revisedFile->getAssocType();

                // Copy the assoc id.
                if (!is_null($assocId) && $assocId !== $revisedFile->getAssocId())
                    fatalError('Invalid submission file assoc ID!');
